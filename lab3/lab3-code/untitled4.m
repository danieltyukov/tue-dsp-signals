...
% Preallocate arrays to store empirical and theoretical rho_hat values
rho_hat_empirical = zeros(1, N_max);
rho_hat_theoretical = zeros(1, N_max);
...
for N = 1:N_max
    ...
    rho_hat_empirical(N) = compute_rho_hat(x1, yN);
    
    % Compute the theoretical normalized cross-covariance coefficient
    rho_hat_theoretical(N) = (1 - N) / sqrt(2 - 2 * N + N^2);
end

% Plot the empirical results
figure;
plot(1:N_max, rho_hat_empirical, '-b', 'LineWidth', 1);  % Empirical rho_hat for N = 1:100
hold on;

% Plot the theoretical results
plot(1:N_max, rho_hat_theoretical, '--r', 'LineWidth', 1.5);  % Theoretical rho_hat for N = 1:100

% Highlight specific points at N = 1, 2, 5, 100
for i = 1:length(highlight_N)
    N = highlight_N(i);
    plot(N, rho_hat_empirical(N), 'bo', 'MarkerSize', 4, 'MarkerFaceColor', 'b');  % Highlight empirical points
end

% Add labels and title
xlabel('N', 'FontSize', 12);
ylabel('\rho_{x_1,y_N}[0]', 'FontSize', 12);
%title('Empirical vs Theoretical Normalized Cross-Covariance Coefficient \rho_{x_1,y_N}[0]', 'FontSize', 14);

% Add grid and legend
grid on;
legend('Empirical \rho_{x_1, y_N}[0]', 'Theoretical \rho_{x_1, y_N}[0]', 'Points N = 1, 2, 5, 100', 'Location', 'best');
